...
class brucellosis(ControlledDynamics):
    def __init__(self,
        ...
        """
        super().__init__(['concataned state of shape [7 x 1],' +
                          'with state vars in sequence: susceptible (S), infected (I), brucella (B), '
                          'susceptible_h (S_h), infected_h (I_h), chronic (I_ch), '
                          'infected_x (I_x),infected_h_x (I_h_x)' +
                          'containment (X)'])
    ...
    def forward(self, t, x, u=None):
        ...
        beta_s = u_hat[0, 1].unsqueeze(-1).unsqueeze(-1)
        beta_sw = u_hat[0, 2].unsqueeze(-1).unsqueeze(-1)
        beta_h = u_hat[0, 3].unsqueeze(-1).unsqueeze(-1)
        beta_hw = u_hat[0, 4].unsqueeze(-1).unsqueeze(-1)

        # XiShu = [1, 0, 1, 1, 1e-3, 1e-3]  # c theta betas betasb betah betahb   有负数
        '''x0 = torch.tensor([[
            1749e4, 2.97e4, 4e4, 2560e4, 46, 46 * 0.385, 2.974e4, 46
        ]])'''
        # XiShu = [1e-1, 0, 1e-1, 1, 1e-3, 1e-3]  # c0、beta2-0 为负数
        XiShu = [1e-1, 0, 1e-1, 1, 1e-3, 1e-3]
        N_h = S_h + I_h + I_ch
        N_s = S + I

        dS = self.A - beta_s * S * I * XiShu[2]/N_s - beta_sw * S * B * XiShu[3]/N_s - self.d_s * S
        dI = beta_s * S * I * XiShu[2]/N_s + beta_sw * S * B * XiShu[3]/N_s - self.d_s * I - c * I*XiShu[0]
        dB = self.kappa * I - self.delta * B - self.ntao * B
        dS_h = self.B - beta_h * S_h * I * XiShu[4]/N_h - beta_hw * S_h * B * XiShu[5]/N_h + (1-self.qgamma) * I_h - self.d_h * S_h
        dI_h = beta_h * S_h * I * XiShu[4]/N_h + beta_hw * S_h * B *XiShu[5]/N_h - I_h - self.d_h * I_h
        dI_ch = self.qgamma * I_h - self.d_h * I_ch
        dI_x = beta_s * S * I * XiShu[2]/N_s + beta_sw * S * B * XiShu[3]/N_s
        dI_h_x = beta_h * S_h * I * XiShu[4]/N_h + beta_hw * S_h * B * XiShu[5]/N_h

        # stack derivatives to state in received order
        dx = torch.cat([dS, dI, dB.unsqueeze(-1), dS_h, dI_h, dI_ch.unsqueeze(-1),
                       dI_x, dI_h_x], dim=-1)
        return dx

def evaluate_trajectory(dynamics, controller, x0, total_time, n_timesteps, method='rk4', options=None):
    all_controls = []
    all_control_times = []
    all_timesteps = torch.linspace(0, 5, 6)
