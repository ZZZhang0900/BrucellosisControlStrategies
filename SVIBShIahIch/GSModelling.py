...
class brucellosis(ControlledDynamics):
    def __init__(self,
        ...
        """
        super().__init__(['concataned state of shape [9 x 1],' +
                          ...
                          'vaccinated_x (V_x),infected_x (I_x),infected_h_x (I_h_x)' +
                          'containment (X)'])
    ...
    def forward(self, t, x, u=None):
        ...
        #XiShu = [1, 1, 1, 1, 1, 1]
        XiShu = [1e-2, 1, 1e-1, 1e-1, 1e-3, 1e-3]  #880次，loss=0.0176，全正，B=4e5；1507次，loss=0.0125，全正，B=2e5
        N_h = S_h + I_h + I_ch
        N_s = S + I + V

        dS = self.A - beta_s * S * I *XiShu[2]/N_s - beta_sw * S * B  *XiShu[3]/N_s- theta * S *XiShu[1]+ self.lamda * V - self.d_s * S
        dI = beta_s * S * I *XiShu[2]/N_s + beta_sw * S * B *XiShu[3]/N_s - self.d_s * I - c * I*XiShu[0]
        dV = theta * S *XiShu[1]- self.lamda * V - self.d_s * V
...
def evaluate_trajectory(dynamics, controller, x0, total_time, n_timesteps, method='rk4',
    ...
    all_controls = []
    all_control_times = []
    all_timesteps = torch.linspace(0, 4, 5)
